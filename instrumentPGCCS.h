	...
	std::tuple<double, std::vector<double>>  pGLinZerosCurveSequential(std::map<int, double, std::less<int>>& points_values, int numOfFixings, int curveSpot, Curve &oisCurve) const
	{
		...
		int adg = instrument.fixedTenor.front() - '0';
		int sfh = instrument.floatTenor.front() - '0';
		assert(adg >= sfh);
		int alpha = adg / sfh; // 1 if 3M-3M or 2 if 6M-3M etc.
		py::print("alpah", alpha);
		...
		for (int i = 1, e = instrument.numofPeriods; i <= e; ++i)
		{
			...
			int auxpaydate = paydate;
			if (instrument.floatTenor == "3M" || instrument.fixedTenor == "3M")
			{
				paydate = shiftByThreeMonths(paydate, false);
				py::print("3M3M or 3M6M");
			}

			else {
				if (instrument.floatTenor == "6M" || instrument.fixedTenor == "6M")
			...
			for (auto k = points_values.begin(), e = points_values.end(); k != e; ++k)
			{
				...
				if (foundfirst_cpdate != 0)
				{
					...
					if (paydate <= k->first)
					{
						...
						{
							py::print(foundfirst_cpdate, k->first, foundfirst_cpvalue, k->second, paydate, curveSpot);
	...
	std::tuple<double, std::vector<double>>  pGLinZerosCurveSequentialFFS(std::map<int, double, std::less<int>>& points_values, int numOfFixings, bool finalReturn, double spotFxRate, int curveSpot, Curve &oisCurve, Curve &foreignOIS, Curve &Libor) const
	{
		double pvFixedLeg = 0.0;
		double pvFloatLeg = 0.0;
		...
		SwapInstrument anInst(instrument.floatTenor, instrument.fixedTenor, "plain", "flofix",
			...
			instrument.spreadFloat, finalReturn);

		double pvdomFloatLeg = instrumentPrice(anInst, Libor, oisCurve, true);



		std::tuple<double, std::vector<double>> result(-pvFixedLeg + pvFloatLeg + pvdomFloatLeg, Gradient);
		return result;
	}//


	//THIS GUY SELECTS FROM THE ONES ABOVE
	std::tuple<double, std::vector<double>> defaultPG(std::map<int, double, std::less<int>>& points_values, int numOfFixings, bool finalReturn, double spotFxRate, int curveSpot, Curve &foreignOIS, Curve &Libor, Curve &domOIS) const
	{
		...
		{
			if (instrument.paymType == "flofix")  return pGLinZerosCurveSequential(points_values, numOfFixings, curveSpot, foreignOIS);
			else if (instrument.paymType == "floflo")  return pGLinZerosCurveSequentialFFS(points_values, numOfFixings, finalReturn, spotFxRate, curveSpot, foreignOIS, Libor, domOIS);
